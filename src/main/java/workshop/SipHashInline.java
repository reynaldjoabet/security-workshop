 ...
 *
 * @see <a href=
 *      "https://github.com/nahi/siphash-java-inline">https://github.com/nahi/siphash-java-inline</a>
 */
public class SipHashInline {
	...
	public static long hash24(long k0, long k1, byte[] data, int offset, int len, boolean reversed) {
		...
		while (i < last) {
			 ...
			 *
			 * SIPROUND in siphash24.c: A: v0 += v1; B: v1=ROTL(v1,13); C: v1 ^= v0; D:
			 * v0=ROTL(v0,32); E: v2 += v3; F: v3=ROTL(v3,16); G: v3 ^= v2; H: v0 += v3; I:
			 * v3=ROTL(v3,21); J: v3 ^= v0; K: v2 += v1; L: v1=ROTL(v1,17); M: v1 ^= v2; N:
			 * v2=ROTL(v2,32);
			 *
			 * Each dependency: B -> A C -> A, B D -> C F -> E G -> E, F H -> D, G I -> H J
			 * -> H, I K -> C, G L -> K M -> K, L N -> M
			 *
			 * Dependency graph: D -> C -> B -> A G -> F -> E J -> I -> H -> D, G N -> M ->
			 * L -> K -> C, G
			 *
	 ...
	 * reversed input in order to minimize the risk of collisions (for a collision
	 * to be an actual value, it must have the same structure as the original input,
	 * i.e. be a palindrome).
	 *
	 * @param k0     the k 0
	 * @param k1     the k 1
	...
	public static long hash24_palindromic(long k0, long k1, byte[] data, int offset, int len) {
		...
		if (last > 0) {
			...
			{
				// Use conditional only when around the end of the buffer, otherwise this is too
				// slow
		...
		if (0 != idx) {
			// idx != 0 means we have already consumed part of the buffer going forward and
			// backward, i.e. 2*len >= 8
			int i = 0;
			while (i < (len * 2 - idx)) {
				m <<= 8;
