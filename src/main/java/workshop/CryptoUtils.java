...
public interface KeyStore {
	...
	public static byte[] addMAC(long[] key, byte[] data) {
		...
		System.arraycopy(longToBytes(hash), 0, authenticated, data.length, 8);
	...
	/**
	 * Check the MAC and return the verified content or null if the verification
	 * failed
	 *
	 * @param key  the key
	 * @param data the data
	...
	public static byte[] removeMAC(byte[] key, byte[] data) {
		...
		long mac = longFromBytes(Arrays.copyOfRange(data, data.length - 8, data.length));
	...
	/**
	 * Compute a safe SipHash of a given input by computing the forward hash, the
	 * hash of the reversed input and finally the hash of the two concatenated
	 * hashes.
	 * <p>
	 * This should prevent having meaningful collisions. If two contents have
	 * colliding hashes, this means that the concatenation of their forward and
	 * reverse hashes are collisions for SipHah, quite unlikely.
	 *
	 * @param k0  the k 0
	 * @param k1  the k 1
	 * @param msg the msg
	...
	public static byte[] decodeKey(KeyStore ks, String encoded) {
		...
		if (encoded.startsWith("hex:")) {
			...
			System.arraycopy(longToBytes(SipHashInline.hash24_palindromic(SALT_K0, SALT_K1, key)), 0, salt, 0, 8);
			System.arraycopy(longToBytes(SipHashInline.hash24_palindromic(SALT_K1, SALT_K0, key)), 0, salt, 8, 8);

			byte[] secret = longToBytes(SipHashInline.hash24_palindromic(SECRET_K0, SECRET_K1, key));
			byte[] additional = longToBytes(SipHashInline.hash24_palindromic(ADDITIONAL_K0, ADDITIONAL_K1, key));

			Argon2Parameters.Builder builder = new Argon2Parameters.Builder(Argon2Parameters.ARGON2_id).withSalt(salt)
					.withSecret(secret).withAdditional(additional).withVersion(Argon2Parameters.ARGON2_VERSION_13)
					.withIterations(iters).withMemoryAsKB(memory).withParallelism(parallelism);
