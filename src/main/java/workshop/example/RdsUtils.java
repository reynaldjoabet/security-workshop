...
public class RdsUtils {
	...
	// Aurora DB clusters support different endpoints. More details about Aurora RDS
	// endpoints
	// can be found at
	...
	// <database-cluster-name>.cluster-<xyz>.<aws-region>.rds.amazonaws.com
	...
	// Example:
	// test-postgres-alias.cluster-custom-123456789012.us-east-2.rds.amazonaws.com
	//
	...
	//
	//
	// Similar endpoints for China regions have different structure and are
	// presented below.
	...
	// Example:
	// test-postgres.cluster-ro-123456789012.rds.cn-northwest-1.amazonaws.com.cn
	//
	// Cluster Custom Endpoint:
	// <cluster-name-alias>.cluster-custom-<xyz>.rds.<aws-region>.amazonaws.com.cn
	// Example:
	// test-postgres-alias.cluster-custom-123456789012.rds.cn-northwest-1.amazonaws.com.cn
	//
	// Instance Endpoint: <instance-name>.<xyz>.rds.<aws-region>.amazonaws.com.cn
	// Example:
	// test-postgres-instance-1.123456789012.rds.cn-northwest-1.amazonaws.com.cn
	//
	//
	//
	...
	// Example:
	// test-custom-endpoint-name.endpoint.proxy-123456789012.us-east-2.rds.amazonaws.com

	private static final Pattern AURORA_DNS_PATTERN = Pattern
			.compile("^(?<instance>.+)\\." + "(?<dns>proxy-|cluster-|cluster-ro-|cluster-custom-|shardgrp-)?"
					+ "(?<domain>[a-zA-Z0-9]+\\.(?<region>[a-zA-Z0-9\\-]+)"
					+ "\\.(rds|rds-fips)\\.amazonaws\\.(com|au|eu|uk)\\.?)$", Pattern.CASE_INSENSITIVE);

	private static final Pattern AURORA_CLUSTER_PATTERN = Pattern.compile("^(?<instance>.+)\\."
			+ "(?<dns>cluster-|cluster-ro-)+" + "(?<domain>[a-zA-Z0-9]+\\.(?<region>[a-zA-Z0-9\\-]+)"
			+ "\\.(rds|rds-fips)\\.amazonaws\\.(com|au|eu|uk)\\.?)$", Pattern.CASE_INSENSITIVE);
	private static final Pattern AURORA_LIMITLESS_CLUSTER_PATTERN = Pattern.compile("(?<instance>.+)\\."
			+ "(?<dns>shardgrp-)+" + "(?<domain>[a-zA-Z0-9]+\\.(?<region>[a-zA-Z0-9\\-]+)"
			+ "\\.(rds|rds-fips)\\.(amazonaws\\.com\\.?|amazonaws\\.eu\\.?|amazonaws\\.au\\.?|amazonaws\\.uk\\.?"
			+ "|amazonaws\\.com\\.cn\\.?|sc2s\\.sgov\\.gov\\.?|c2s\\.ic\\.gov\\.?))$", Pattern.CASE_INSENSITIVE);
	private static final Pattern AURORA_CHINA_DNS_PATTERN = Pattern.compile("^(?<instance>.+)\\."
			+ "(?<dns>proxy-|cluster-|cluster-ro-|cluster-custom-|shardgrp-)?"
			+ "(?<domain>[a-zA-Z0-9]+\\.(rds|rds-fips)\\.(?<region>[a-zA-Z0-9\\-]+)" + "\\.amazonaws\\.com\\.cn\\.?)$",
			Pattern.CASE_INSENSITIVE);

	private static final Pattern AURORA_CHINA_CLUSTER_PATTERN = Pattern.compile("^(?<instance>.+)\\."
			+ "(?<dns>cluster-|cluster-ro-)+" + "(?<domain>[a-zA-Z0-9]+\\.(rds|rds-fips)\\.(?<region>[a-zA-Z0-9\\-]+)"
			+ "\\.amazonaws\\.com\\.cn\\.?)$", Pattern.CASE_INSENSITIVE);

	private static final Pattern AURORA_OLD_CHINA_DNS_PATTERN = Pattern.compile("^(?<instance>.+)\\."
			+ "(?<dns>proxy-|cluster-|cluster-ro-|cluster-custom-|shardgrp-)?"
			+ "(?<domain>[a-zA-Z0-9]+\\.(?<region>[a-zA-Z0-9\\-]+)" + "\\.(rds|rds-fips)\\.amazonaws\\.com\\.cn\\.?)$",
			Pattern.CASE_INSENSITIVE);

	private static final Pattern AURORA_OLD_CHINA_CLUSTER_PATTERN = Pattern.compile("^(?<instance>.+)\\."
			+ "(?<dns>cluster-|cluster-ro-)+" + "(?<domain>[a-zA-Z0-9]+\\.(?<region>[a-zA-Z0-9\\-]+)"
			+ "\\.(rds|rds-fips)\\.amazonaws\\.com\\.cn\\.?)$", Pattern.CASE_INSENSITIVE);

	private static final Pattern AURORA_GOV_DNS_PATTERN = Pattern
			.compile(
					"^(?<instance>.+)\\." + "(?<dns>proxy-|cluster-|cluster-ro-|cluster-custom-|shardgrp-)?"
							+ "(?<domain>[a-zA-Z0-9]+\\.(rds|rds-fips)\\.(?<region>[a-zA-Z0-9\\-]+)"
							+ "\\.(amazonaws\\.com\\.?|c2s\\.ic\\.gov\\.?|sc2s\\.sgov\\.gov\\.?))$",
					Pattern.CASE_INSENSITIVE);

	private static final Pattern AURORA_GOV_CLUSTER_PATTERN = Pattern.compile("^(?<instance>.+)\\."
			+ "(?<dns>cluster-|cluster-ro-)+" + "(?<domain>[a-zA-Z0-9]+\\.(rds|rds-fips)\\.(?<region>[a-zA-Z0-9\\-]+)"
			+ "\\.(amazonaws\\.com\\.?|c2s\\.ic\\.gov\\.?|sc2s\\.sgov\\.gov\\.?))$", Pattern.CASE_INSENSITIVE);

	private static final Pattern ELB_PATTERN = Pattern.compile(
			"^(?<instance>.+)\\.elb\\." + "((?<region>[a-zA-Z0-9\\-]+)\\.amazonaws\\.(com|au|eu|uk)\\.?)$",
			Pattern.CASE_INSENSITIVE);
	...
	private static final Pattern IP_V6_COMPRESSED = Pattern.compile(
			"^(([0-9A-Fa-f]{1,4}(:[0-9A-Fa-f]{1,4}){0,5})?)" + "::(([0-9A-Fa-f]{1,4}(:[0-9A-Fa-f]{1,4}){0,5})?)$");

	private static final Pattern BG_GREEN_HOST_PATTERN = Pattern.compile(".*(?<prefix>-green-[0-9a-z]{6})\\..*",
			Pattern.CASE_INSENSITIVE);

	private static final Pattern BG_GREEN_HOSTID_PATTERN = Pattern.compile("(.*)-green-[0-9a-z]{6}",
			Pattern.CASE_INSENSITIVE);

	private static final Pattern BG_OLD_HOST_PATTERN = Pattern.compile(".*(?<prefix>-old1)\\..*",
			Pattern.CASE_INSENSITIVE);

	// https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/Concepts.Aurora_Fea_Regions_DB-eng.Feature.GlobalDatabase.html
	private static final Pattern AURORA_GLOBAL_WRITER_DNS_PATTERN = Pattern
			.compile(
					"^(?<instance>.+)\\." + "(?<dns>global-)?"
							+ "(?<domain>[a-zA-Z0-9]+\\.global\\.rds\\.amazonaws\\.com\\.?)$",
					Pattern.CASE_INSENSITIVE);

	private static final Pattern RDS_PROXY_ENDPOINT_DNS_PATTERN = Pattern
			.compile(
					"^(?<instance>.+)\\.endpoint\\." + "(?<dns>proxy-)?"
							+ "(?<domain>[a-zA-Z0-9]+\\.(?<region>[a-zA-Z0-9\\-]+)" + "\\.rds\\.amazonaws\\.com\\.?)$",
					Pattern.CASE_INSENSITIVE);

	private static final Pattern RDS_PROXY_ENDPOINT_CHINA_DNS_PATTERN = Pattern.compile(
			"^(?<instance>.+)\\.endpoint\\." + "(?<dns>proxy-)+"
					+ "(?<domain>[a-zA-Z0-9]+\\.rds\\.(?<region>[a-zA-Z0-9\\-]+)" + "\\.amazonaws\\.com\\.cn\\.?)$",
			Pattern.CASE_INSENSITIVE);

	private static final Pattern RDS_PROXY_ENDPOINT_OLD_CHINA_DNS_PATTERN = Pattern.compile(
			"^(?<instance>.+)\\.endpoint\\." + "(?<dns>proxy-)?" + "(?<domain>[a-zA-Z0-9]+\\.(?<region>[a-zA-Z0-9\\-]+)"
	...
	public boolean isRdsDns(final String host) {
		...
		final Matcher matcher = cacheMatcher(preparedHost, AURORA_DNS_PATTERN, AURORA_CHINA_DNS_PATTERN,
				AURORA_OLD_CHINA_DNS_PATTERN, AURORA_GOV_DNS_PATTERN);
	...
	public boolean isRdsProxyEndpointDns(final String host) {
		...
		final Matcher matcher = cacheMatcher(preparedHost, RDS_PROXY_ENDPOINT_DNS_PATTERN,
				RDS_PROXY_ENDPOINT_CHINA_DNS_PATTERN, RDS_PROXY_ENDPOINT_OLD_CHINA_DNS_PATTERN);
	...
	public String getRdsClusterId(final String host) {
		final String preparedHost = getPreparedHost(host);
		if (StringUtils.isNullOrEmpty(preparedHost)) {
			return null;
		}
	...
	public String getRdsInstanceId(final String host) {
		final String preparedHost = getPreparedHost(host);
		if (StringUtils.isNullOrEmpty(preparedHost)) {
			return null;
		}
	...
	public boolean isSameRegion(final String host1, final String host2) {
	...
	public String getRdsClusterHostUrl(final String host) {
		...
		if (limitlessMatcher.find()) {
			return preparedHost.replaceAll(AURORA_LIMITLESS_CLUSTER_PATTERN.pattern(),
					"${instance}.shardgrp-${domain}");
		}
		return null;
	}
	...
	// Verify that provided host is a blue host name and contains neither green
	// prefix nor old prefix.
	...
	private String getDnsGroup(final String host) {
		...
		return cachedDnsPatterns.computeIfAbsent(host, (k) -> {
			final Matcher matcher = cacheMatcher(k, AURORA_DNS_PATTERN, AURORA_CHINA_DNS_PATTERN,
					AURORA_OLD_CHINA_DNS_PATTERN, AURORA_GOV_DNS_PATTERN, AURORA_GLOBAL_WRITER_DNS_PATTERN);
